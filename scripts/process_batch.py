...
def main():
    ...
    for _, r in df.iterrows():
        ...
        add_basic_qc(adata)
        qc_rows.append(qc_summary(adata))
        ...
        adata_f.write(args.outdir / f"{run.run_id}.filtered.h5ad")
        print(f"[ok] {run.run_id}")

    pd.DataFrame(qc_rows).to_csv(args.outdir / "qc_summary_all_runs.csv", index=False)
